...
def save_augmented_image(image, filename, save_dir="augmented_images"):
...
class LoadImagesAndFakeLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            img = augment_v(img, vgain=hyp['hsv_v'])

            img, labels = scale_augmentation(img, labels, scale_range=(0.9, 1.1)) 
            ...
            if random.random() < 0.5:
                if(len(labels) > 0):
                   img, labels = blur_augmentation(img, labels)

            # 증강된 이미지 저장
            if index == 0:  # 첫 번째 이미지만 저장
                save_augmented_image(img, f"augmented_{index}.jpg")
...
def augment_v(img,vgain=0.5):
    r = np.random.uniform(-1, 1, 3) * [vgain] + 1  # random gains
    hue, sat, val = cv2.split(cv2.cvtColor(img, cv2.COLOR_BGR2HSV))
    dtype = img.dtype  # uint8
...
def scale_augmentation(img, labels, scale_range=(0.9, 1.1)):
    h, w = img.shape[:2]
    ...
    if scale_factor > 1.0:
        # If the new image is larger, we need to center-crop it
        start_x = (new_w - w) // 2
        start_y = (new_h - h) // 2
        img = img[start_y:start_y + h, start_x:start_x + w]
    else:
        # If the new image is smaller, we need to pad it
        new_img = np.full((h, w, 3), 114, dtype=np.uint8)
        start_x = (w - new_w) // 2
        start_y = (h - new_h) // 2
    ...
    if labels.size > 0:
        ...
        labels[:, 1::2] = np.clip(labels[:, 1::2], 0, w)  # clip x
        labels[:, 2::2] = np.clip(labels[:, 2::2], 0, h)  # clip y
...
def blur_augmentation(image, labels):
    # Applies image blur augmentation to random regions
    ...
    for s in scales:
        ...
        image[ymin:ymax, xmin:xmax] = cv2.GaussianBlur(image[ymin:ymax, xmin:xmax], (15, 15), 0)
