...
from utils.loggers.wandb.wandb_utils import check_wandb_resume, WandbLogger
...
from utils.datasets_ssod import create_target_dataloader, augment_v, blur_augmentation
...
class SSODTrainer(Trainer):
    ...
    def build_model(self, cfg, device, resume=True):
        ...
        if pretrained:
            ...
            exclude = ['anchor'] if (cfg or cfg.Model.anchors) and not resume else []  # exclude keys
        ...
        if pretrained:
            ...
            if self.semi_ema and ckpt.get('semi_ema'):
                self.semi_ema.ema.load_state_dict(ckpt['semi_ema'].float().state_dict(), strict=False)
            # EMA
            # if self.ema and ckpt.get('ema'):
            #     self.ema.ema.load_state_dict(ckpt['ema'].float().state_dict(), strict=False)
            #     self.ema.updates = ckpt['updates']
            print("Resume training setting:", resume)
            # cfg.resume=True
            # Epochs
            self.start_epoch = ckpt['epoch'] + 1
            # print(f'????????????{self.start_epoch}')
            if resume:
    ...
    def after_epoch(self, callbacks, val):
        ...
        if self.RANK in [-1, 0]:
            ...
            if not self.noval or final_epoch:  # Calculate mAP
                ...
                # class_names = self.cfg.Dataset.names
                # for i, class_name in enumerate(class_names):
                #     wandb.log({f"{class_name}_mAP": maps[i]})

                # # 전체 성능 지표도 WandB에 로그
                # wandb.log({"Overall_mAP": self.results[2]})
                # wandb.log({"Overall_mAP_0.5:0.95": self.results[3]})

                # # 혼동 행렬 계산 및 시각화
                # num_classes = len(self.cfg.Dataset.names)
                # print(f'3 : {num_classes}')

                # cm = self.calculate_confusion_matrix_metrics(all_predictions, all_targets, num_classes)
                # self.plot_confusion_matrix(cm, self.cfg.Dataset.names)

                # # 혼동 행렬 이미지를 WandB에 업로드
            ...
            # confusion_matrix_path = str(self.save_dir / 'confusion_matrix.png')
            # if self.wandb_logger and os.path.exists(confusion_matrix_path):  # wandb_logger 객체가 존재하고 파일이 존재하면
            #     self.wandb_logger.log_confusion_matrix(confusion_matrix_path)
            #     print(f"Logged confusion matrix for epoch {self.epoch}")
            ...
            if (not self.nosave) or (final_epoch):  # if save
                if self.epoch >= self.cfg.hyp.burn_epochs:
                    ckpt = {'epoch': self.epoch,
                        ...
                        'semi_ema': deepcopy(self.semi_ema.ema).half(),
            ...
            # Save last
                torch.save(ckpt, self.save_dir / 'ema_last.pt')
                torch.save(ckpt_ssod, self.save_dir / 'ssod_last.pt')
    ...
    def train_without_unlabeled(self, callbacks):
        if self.pretrained:
            LOGGER.info("Skipping supervised training as pretrained weights")
            return
            ...
            for f in self.last, self.best:
                if f.exists():
                    ...
                    if f is self.best:
                        LOGGER.info(f'\nValidating {f}...')
                        # val_ssod = self.cfg.SSOD.train_domain
                        results, maps, _, _ = val.run(self.data_dict,
                                            batch_size=self.batch_size // self.WORLD_SIZE * 2,
                                            imgsz=self.imgsz,
