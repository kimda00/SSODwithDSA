...
# from utils.loggers.wandb.wandb_utils import plot_confusion_matrix
...
def run(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            # # 디버깅을 위해 추가된 부분
            # if len(tcls) != len(pred_classes):
            #     print(f"Warning: Inconsistent lengths at image {paths[si]}: y_true({len(tcls)}) != y_pred({len(pred_classes)})")
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / './dsa_yolov7_ssod/exp36/weights/best.pt', help='model.pt path(s)')
