...
# # TensorBoard 로그 파일 경로 설정
# log_dir = './dsa_yolov7_ssod/exp36'  # TensorBoard 로그 파일 경로로 변경

# # TensorBoard 로그 파일에서 사용 가능한 태그 확인
# def check_available_tags(log_dir):
...
#                 available_tags.add(v.tag)

#     return available_tags

# # 사용 가능한 태그 출력
# available_tags = check_available_tags(log_dir)
...
# # def extract_data_from_tensorboard(log_dir):
# #     event_files = [os.path.join(log_dir, f) for f in os.listdir(log_dir) if 'events.out.tfevents' in f]
# #     all_preds = []
# #     all_labels = []
...
# #         for e in tf.compat.v1.train.summary_iterator(event_file):
# #             for v in e.summary.value:
# #                 if v.tag == 'predictions':
# #                     all_preds.append(tf.make_ndarray(v.tensor))
# #                 elif v.tag == 'labels':
# #                     all_labels.append(tf.make_ndarray(v.tensor))
...
# # # 데이터 추출
# # preds, labels = extract_data_from_tensorboard(log_dir)
